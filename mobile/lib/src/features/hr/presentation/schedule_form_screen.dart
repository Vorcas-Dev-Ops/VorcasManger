...
class ScheduleFormScreen extends ConsumerStatefulWidget {
  ...
  const ScheduleFormScreen({super.key, this.initialDate});
        ...
        child: ListView(
          ...
          children: [
            ...
            _buildTextField(
              ...
              label: 'Description',
              hint: 'e.g., Discussing Q1 results',
            ...
            // Add Search
            const Text('Search & Assign Staff', style: TextStyle(color: SuperAdminTheme.textSecondary, fontSize: 13, fontWeight: FontWeight.bold)),
            const SizedBox(height: 8),
            TextFormField(
                ...
                child: employeesState.when(
                  data: (employees) {
                    final results = employees.where((e) {
                      ...
                      final callerLevel = currentUser?.hierarchyLevel ?? 99;
                      final empLevel = e.hierarchyLevel ?? 99;
                      
                      // Only show employees with a HIGHER hierarchy number (meaning LOWER rank)
                      // e.g., if I am Level 3 (HR), I see Level 4 (TL) and Level 5 (Employee)
                      final isSubordinate = empLevel > callerLevel;
                      
                      return matchesSearch && notAlreadySelected && isSubordinate;
                    }).toList();

                    if (results.isEmpty) {
                      return const Padding(
                        padding: EdgeInsets.all(16.0),
                        child: Text('No matching employees found', style: TextStyle(color: SuperAdminTheme.textSecondary, fontSize: 12)),
                      );
                    }
